...
export const getPancakeTokenURL = function (token, v1 = false) {
  ...
  return `https://pancakeswap.finance/#/swap?outputCurrency=${token}`;
};
...
export const getBscScanAddressUrl = function (address) {
  if (process.env.NETWORK === 'testnet') {
...
export const getWeb3Connection = function () {
  const wssnode = process.env.WSS_NODE;
  if (!wssnode) {
    throw ('Input wss node');
  }
  return new Web3(wssnode);
};
...
* Get wallets to watch
*
* @method getNamesWallet
* @return {Array} name
*/
export const getNamesWallet = function () {
  return readFileSync('.walletnames').toString().replace(/\r\n/g, '\n').split('\n');
};
 ...
 * @return {Boolean}
*/
export const isPancakeSwapV2Router = function (router) {
  return router.toLowerCase() === '0x10ed43c718714eb63d5aa57b78b54704e256024e';
};